...
public class BleService extends Service {
    ...
    public void onCreate() {
        ...
        try {
            ...
            // Surface any initialization errors to the UI and logs so we can diagnose crashes.
            Log.e("BleService", "onCreate failed", t);
            if (getApplicationContext() != null) {
                try {
                    Bus.sendError(getApplicationContext(), "BLE init error: " + t.getClass().getSimpleName() + ": " + t.getMessage());
                } catch (Throwable ignore) {
                }
            }
            // Re-throw to preserve original behavior if desired, but keep the app alive for debugging.
        }
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        try {
            ...
            Log.e("BleService", "onStartCommand failed", t);
            try {
                Bus.sendError(this, "BLE start error: " + t.getClass().getSimpleName() + ": " + t.getMessage());
            } catch (Throwable ignore) {}
        }
        return START_STICKY;
    }
    ...
    private void connect(String mac) {
        ...
        // On Android 12+ connecting to BLE devices requires BLUETOOTH_CONNECT permission.
        if (checkSelfPermission(android.Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
            Bus.sendError(this, "Missing BLUETOOTH_CONNECT permission");
            return;
        }
